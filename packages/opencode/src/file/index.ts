...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const mkdir = Effect.fn("File.mkdir")(function* (dir: string) {
      const ctx = yield* InstanceState.context
      const next = path.join(ctx.directory, dir)
      if (!containsPath(next, ctx)) throw new Error("Access denied: path escapes project directory")
      yield* appFs.makeDirectory(next, { recursive: true })
    })

    const rename = Effect.fn("File.rename")(function* (input: { from: string; to: string }) {
      const ctx = yield* InstanceState.context
      const from = path.join(ctx.directory, input.from)
      const to = path.join(ctx.directory, input.to)
      if (!containsPath(from, ctx)) throw new Error("Access denied: path escapes project directory")
      if (!containsPath(to, ctx)) throw new Error("Access denied: path escapes project directory")
      if (from === ctx.directory) throw new Error("Cannot rename root directory")
    ...
    const rmdir = Effect.fn("File.rmdir")(function* (dir: string) {
      ...
      const next = path.join(ctx.directory, dir)
      if (!containsPath(next, ctx)) throw new Error("Access denied: path escapes project directory")
      if (next === ctx.directory) throw new Error("Cannot remove root directory")
